        ...
        if (headings.length > 0) {
            ...
            btnTopo.innerHTML = `
                <div class="toc-text-wrapper">
                    <div class="toc-text">Voltar ao topo</div>
                </div>
...
async function handleAiSubmit() {
    ...
    try {
        const response = await fetch('pt_api/chat.php', {
            method: 'POST',
        ...
        loadingDiv.remove();
        appendAiMessage('bot', data.reply || "Tô tendo problemas pra responder agora. Vamos tentar de novo mais tarde?");
    } catch (err) {
        clearInterval(loadingDiv._dotInterval);
        loadingDiv.remove();
        appendAiMessage('bot', "A conexão com o servidor de IA falhou. Tenta de novo mais tarde, beleza?");
